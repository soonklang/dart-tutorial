...
คลาส **File** มีเมธอด **create()** ที่สามารถใช้สร้างไฟล์ใหม่ใน directory ที่ต้องการได้ 
โดยวิธีนี้จะต้องระบุชื่อ directory ที่ต้องการจะสร้างไฟล์และชื่อของไฟล์ที่ต้องการจะสร้าง
...
void main() {
  final directory = Directory('my_files');
  final file = File('sample.txt');
  file.create(parent: directory);
}
```
